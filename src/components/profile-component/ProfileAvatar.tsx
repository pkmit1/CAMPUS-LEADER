...
export default function ProfileAvatar({ user, setUser }: Props) {
  ...
  const handleImageUpload = async () => {
    ...
    try {
      ...
      setUser({ ...user, image: res.data.imageUrl });
      setFile(null);
      setPreview("");
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg p-6 flex flex-col items-center text-center">
      <div className="relative mb-4 group">
        <Avatar className="h-32 w-32 border-4 border-purple-100 shadow-2xl group-hover:scale-105 transition-transform duration-300">
          <AvatarImage
      ...
      <p className="text-gray-600 text-lg mb-4">{user.bio || "BCA Student"}</p>
